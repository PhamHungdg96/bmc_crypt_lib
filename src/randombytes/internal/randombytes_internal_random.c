...
#define INTERNAL_RANDOM_BLOCK_SIZE 32U
...
static InternalRandomGlobal global = {
    BMC_CRYPT_C99(.initialized =) 0,
    BMC_CRYPT_C99(.random_data_source_fd =) -1
};

static TLS InternalRandom stream = {
    BMC_CRYPT_C99(.initialized =) 0,
    BMC_CRYPT_C99(.rnd32_outleft =) (size_t) 0U
};
...
{
    global.rdrand_available = bmc_crypt_runtime_has_rdrand();
}
...
randombytes_internal_random_close(void)
{
    ...
    bmc_crypt_memzero(&stream, sizeof stream);
...
randombytes_internal_random_buf(void * const buf, const size_t size)
{
    ...
    COMPILER_ASSERT(sizeof stream.nonce == 8U);
...
# endif
#endif
    ret = crypto_core_aes_ctr_ietf((unsigned char *) buf, (unsigned long long) size,
    ...
    stream.nonce++;
    crypto_core_aes_ctr_ietf_xor(stream.key, stream.key, sizeof stream.key,
                                 (unsigned char *) &stream.nonce, stream.key);
}
...
randombytes_internal_random(void)
{
    ...
    if (stream.rnd32_outleft <= (size_t) 0U) {
        randombytes_internal_random_stir_if_needed();
        COMPILER_ASSERT(sizeof stream.nonce == 8U);
        ret = crypto_core_aes_ctr_ietf((unsigned char *) stream.rnd32,
...
struct randombytes_implementation randombytes_internal_implementation = {
    BMC_CRYPT_C99(.implementation_name =) randombytes_internal_implementation_name,
    BMC_CRYPT_C99(.random =) randombytes_internal_random,
    BMC_CRYPT_C99(.stir =) randombytes_internal_random_stir,
    BMC_CRYPT_C99(.uniform =) NULL,
    BMC_CRYPT_C99(.buf =) randombytes_internal_random_buf,
    BMC_CRYPT_C99(.close =) randombytes_internal_random_close
};
