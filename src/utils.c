...
_bmc_crypt_dummy_symbol_to_prevent_memzero_lto(void *const  pnt,
                                            const size_t len);
__attribute__((weak)) void
_bmc_crypt_dummy_symbol_to_prevent_memzero_lto(void *const  pnt,
                                            const size_t len)
...
void
bmc_crypt_memzero(void * const pnt, const size_t len)
{
#if defined(_WIN32) && !defined(__CRT_INLINE)
    SecureZeroMemory(pnt, len);
#elif defined(HAVE_MEMSET_S)
    if (len > 0U && memset_s(pnt, (rsize_t) len, 0, (rsize_t) len) != 0) {
        bmc_crypt_misuse(); /* LCOV_EXCL_LINE */
    }
    ...
    if (len > 0U) {
        memset(pnt, 0, len);
        _bmc_crypt_dummy_symbol_to_prevent_memzero_lto(pnt, len);
    }
...
void
bmc_crypt_stackzero(const size_t len)
{
#ifdef HAVE_C_VARARRAYS
    unsigned char fodder[len];
    bmc_crypt_memzero(fodder, len);
#elif HAVE_ALLOCA
    bmc_crypt_memzero(alloca(len), len);
#endif
}

#ifdef HAVE_WEAK_SYMBOLS
__attribute__((weak)) void
_bmc_crypt_dummy_symbol_to_prevent_memcmp_lto(const unsigned char *b1,
...
int
bmc_crypt_memcmp(const void *const b1_, const void *const b2_, size_t len)
{
#ifdef HAVE_WEAK_SYMBOLS
    ...
    _bmc_crypt_dummy_symbol_to_prevent_memcmp_lto(b1, b2, len);
#endif
    for (i = 0U; i < len; i++) {
        d |= b1[i] ^ b2[i];
    }
...
#ifdef HAVE_WEAK_SYMBOLS
__attribute__((weak)) void
_bmc_crypt_dummy_symbol_to_prevent_compare_lto(const unsigned char *b1,
                                            const unsigned char *b2,
                                            const size_t         len);
__attribute__((weak)) void
_bmc_crypt_dummy_symbol_to_prevent_compare_lto(const unsigned char *b1,
...
int
bmc_crypt_compare(const unsigned char *b1_, const unsigned char *b2_, size_t len)
{
#ifdef HAVE_WEAK_SYMBOLS
...
#if HAVE_WEAK_SYMBOLS
    _bmc_crypt_dummy_symbol_to_prevent_compare_lto(b1, b2, len);
#endif
    i = len;
    while (i != 0U) {
        i--;
...
int
bmc_crypt_is_zero(const unsigned char *n, const size_t nlen)
{
    size_t                 i;
    volatile unsigned char d = 0U;
...
void
bmc_crypt_increment(unsigned char *n, const size_t nlen)
{
    size_t        i = 0U;
    uint_fast16_t c = 1U;
...
void
bmc_crypt_add(unsigned char *a, const unsigned char *b, const size_t len)
{
    size_t        i;
    uint_fast16_t c = 0U;
...
void
bmc_crypt_sub(unsigned char *a, const unsigned char *b, const size_t len)
{
    uint_fast16_t c = 0U;
    size_t        i;
...
int
bmc_crypt_mlock(void *const addr, const size_t len)
{
...
int
bmc_crypt_munlock(void *const addr, const size_t len)
{
    bmc_crypt_memzero(addr, len);
...
_bmc_crypt_malloc(const size_t size)
{
...
# endif
    ...
    (void) bmc_crypt_mlock(unprotected_ptr, unprotected_size); /* not a hard error in the context of bmc_crypt_malloc() */
    canary_ptr =
...
bmc_crypt_malloc(const size_t size)
{
    void *ptr;

    if ((ptr = _bmc_crypt_malloc(size)) == NULL) {
        return NULL;
    }
...
__attribute__((malloc)) void *
bmc_crypt_allocarray(size_t count, size_t size)
{
    if (count > (size_t) 0U && size >= (size_t) SIZE_MAX / count) {
        errno = ENOMEM;
        return NULL;
    }
    return bmc_crypt_malloc(count * size);
}
...
bmc_crypt_free(void *ptr)
{
    ...
    if (bmc_crypt_memcmp(canary_ptr, canary, sizeof canary) != 0) {
        _out_of_bounds();
    }
# ifndef HAVE_PAGE_PROTECTION
    if (bmc_crypt_memcmp(unprotected_ptr + unprotected_size, canary,
                      sizeof canary) != 0) {
        _out_of_bounds();
    }
# endif
    (void) bmc_crypt_munlock(unprotected_ptr, unprotected_size);
...
int
bmc_crypt_mprotect_noaccess(void *ptr)
{
    return _bmc_crypt_mprotect(ptr, _mprotect_noaccess);
}

int
bmc_crypt_mprotect_readonly(void *ptr)
{
    return _bmc_crypt_mprotect(ptr, _mprotect_readonly);
}

int
bmc_crypt_mprotect_readwrite(void *ptr)
{
    return _bmc_crypt_mprotect(ptr, _mprotect_readwrite);
}

int
bmc_crypt_pad(size_t *padded_buflen_p, unsigned char *buf,
...
int
bmc_crypt_unpad(size_t *unpadded_buflen_p, const unsigned char *buf,
